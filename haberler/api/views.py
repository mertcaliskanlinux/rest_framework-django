...
from haberler.api.serializers import MakaleSerializers,GazetecilerSerializer
...
class GazeteciListCreateAPIView(APIView):
    ...
    def get(self,request):
        yazarlar = Gazateci.objects.all()
        serializer = GazetecilerSerializer(yazarlar,many=True,context={'request': request})
        return Response(serializer.data)

    def post(self,request):
        serializer = GazetecilerSerializer(data=request.data)
...
class MakaleDetailAPİView(APIView):
    ...
    def get(self,request,pk):
        ...
        serializer = MakaleSerializers(makale)
        return Response(serializer.data)
        
    def put(self,request,pk):
        makale = self.get_object(pk=pk)
        serializer = MakaleSerializers(makale,data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors,status=status.HTTP_400_BAD_REQUEST)
